...
- [x] **Step 1: 写失败测试**（新建 `stats.spec.ts`）
...
- [x] **Step 1: 写失败测试**（追加到 `stats.spec.ts` 的 describe 内）
...
- [x] **Step 4: 跑测试确认绿**
- [x] **Step 5: 提交** `feat(app3): 学习数据页 · 学习总览 5 个数（与 state 同源）`
...
- [x] **Step 2: 跑测试确认红** → `.st-art` 不存在 / `.hl` 不存在
...
- [x] **Step 4: 跑测试确认绿**
- [x] **Step 5: 提交** `feat(app3): 学习数据页 · 文章学习 6 小卡 + 点卡回首页高亮`
...
- [x] **Step 2: 跑测试确认红** → `.st-num[data-k="learned"]` 不存在
...
- [x] **Step 4: 跑测试确认绿**
- [x] **Step 5: 提交** `feat(app3): 学习数据页 · 单词掌握 4 个数`
...
- [x] **Step 4: 跑测试确认绿**
- [x] **Step 5: 提交** `feat(app3): 学习数据页 · 学习趋势（14 天柱 + SVG 折线，无图表库）`
...
- [x] **Step 2: 跑测试确认红** → `.st-empty[data-empty="listen"]` / `.st-tip` 不存在
...
- [x] **Step 4: 跑测试确认绿**
- [x] **Step 5: 提交** `feat(app3): 学习数据页 · 随身听空态占位 + 待加强（行动导向）`
...
- [x] **Step 2: 跑测试确认红** → 深色那条应红（无 `body.dark` 覆盖时 `.st-num` 仍是 `var(--card)` = 白）；a11y 那条应已绿（Task 1–6 已用 h1/h2 与 44px 按钮）——**若 a11y 已绿属正常，深色必须先红**。
...
- [x] **Step 5: 提交** `feat(app3): 学习数据页收口（深色全覆盖 + 无障碍）+ PRD §5.2/§5.7 同步`
...
## M2 收官记录（2026-09-24，评审修复轮后）

**分支**：`agent/opencode-3.0-m2-0924`（基于 main `4c99978`）。**未合并、未 push**（按要求）。
**交付物**：`#/stats` 学习数据页（未开始态 / 学习总览 / 文章学习 / 单词掌握 / 学习趋势 / 随身听空态 / 待加强）。

### 交付与验证

- 全量 E2E：**226 passed**（M1/M2 基线 215 + M2 的 `stats.spec.ts` 11 条；其中 2 条是评审后补的 blocker 锁与兜底建议锁）。
- `stats.spec.ts` 高重复：`--repeat-each=30` **660/660 全绿**（连续两次）。
- `python3 scripts/validate_data.py` → `Validation PASSED`；`node scripts/build_site.mjs` → 成功。
- `git diff --name-only main...HEAD` 不含 `index.html`、`shadow/**`、`shadow/js/plan-engine.js`、`config.js`。

### 评审修复（送审回来）

| 级别 | 项 | 处置 |
|------|----|------|
| Blocker | 空态建计划后学习数据页不翻面 | `initPlan` 建完计划补 `APP3.route()`（唯一建计划入口，与 `exitTaskMode` 同款出口）；补「建计划后页面自己切到进行中视图」锁，反向验证必红 |
| Important ① | `stats.spec.ts` 播种抢跑 | 加 `waitStatsReady` 闸门：`waitShadowReady` + 等 `#appView` 渲出数据页 + 有界重载重试 |
| Important ② | PRD 三处残留矛盾 | §2.3 / §13.1 / §14.2 改成实装口径 |
| Minor | 过时注释 / plan 复选框 / 补锁 / `_hlArticle` 残留 / SVG 端点椭圆 | 逐条清（详见报告） |

### 过程中发现的真实隐患（非 M2 逻辑，留给 W6/M6）

`/app/` 的 W6 Service Worker 是用 **blob URL** 注册的，拦截 `/app/app.js` 时偶发服务不到这个
defer 脚本（高并发下实测 ~0.3%），`#appView` 会永远空着。本轮在 `app/index.html` 里把 SW 的
fetch 处理器**加固**成「缓存失败一律退回网络」（不再让 `respondWith` 变 rejected），并把测试的
就绪闸门加上有界重载重试，30× 重复已稳定。**正解**是把 SW 落成真实文件 `/app/sw.js` 并加进
`scripts/build_site.mjs` 白名单（本轮改动面不含 `scripts/`，留待 W6/M6）。
