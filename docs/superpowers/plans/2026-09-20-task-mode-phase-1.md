...
| C7 | `loadRoot()` 一进来就采用 `migrate()` 给的默认计划 | **没有 `ielts-task-plan` 时把 `ROOT2.plan` 置空**，设置屏照旧出现 | 引擎对空输入也给一份 15 分钟的计划（纯函数好单测），但"没建过计划"不等于"替他建了一份"——那会把 §9.1 的第一击直接吃掉，也让 `task-flow` 前两条用例在 `.ps-start` 上等到超时 |
| C8 | 面板 `done = 队列里今天已读的句数` | **分母 = `今天已读句数(daily.sentDone) + 队列里未读的句数`** | 队列是滚动的：读完的那句立刻被未读新句顶掉，只数队列里的已读项会永远显示 `0/24`，进度条一动不动 |
| C9 | `stats.newWords` = 全库还没见过面的词数 | **= 今天真的被排进队列的新词数**，原口径改名 `newPool` | 第一天会显示"3245 个新词进队列"，等于把整本词表说成今天的任务 |
| C10 | 「还剩」= 预算 − 已用 | **「还剩」= 未读句的预留成本（句时 + ②③ 各一题）** | 刚开局预算几乎没动，屏幕上是"还剩 12 秒"，说的不是活 |
| C11 | `resetPlan()` 只删老 key | **同时删 `ielts.shadow.v2` 并置 `ROOT2 = null`** | 只清老 key 的话重载后 v2 事件根还在，"清空重来"清不掉任何东西 |
| C12 | 用 `page.route` 把 CDN 换成本地空文件让离线跑得快 | **已试并回退** | 一条用例照样 ~30s，省不了时间；而把 `supabase-js` 换成假对象会让主站 4 条用例（`app-review-queue`、`smoke`）红成"我们的改动坏了" —— 打桩第三方运行时依赖的代价大于收益 |
| C13 | Task 6 交付任务栏三态 | **三态与 `abTitle` 拆分挪到 Task 7**（它依赖 `pass`），Task 6 只删掉副标题里的「已完成 N 轮」 | 半成品三态会让同一块屏出现两套进度；`data-state` 一次到位更安全 |
| C14 | ②③ 直接从 `todayPlan().items` 出题 | **`enterTaskMode()` 时把 items 快照成 `batchItems`**，② 考的是这批快照 | 队列是滚动的：① 读完时队列已经被新句顶光，不快照就会发现 ② 一道题都出不来 |
| C15 | ③ 出不干净四选项就降级成回忆题 | **③ 直接跳过这道题**（`ensureCursor()` 往前挪） | 在 ③ 里默写拼写是 ② 的活；一个屏上出现两种题型，人就不知道自己在被考什么了 |
| C16 | `updateTaskBar` 里继续用 `estimateRemaining()` 两套口径 | **① 态的分母 = `今天已读 + 队列未读`**，`abTitle` 改成 `读到第 N 句 · 章节名`（不带 `x/y`） | 播放条原来的 `第N章 5/61` 与任务栏的 `3/6` 有可能字面撞上，那正是「同一个数字说两遍」的回归 |
| C17 | 键盘 1–4 由题目卡片自己监听 | **`TASK` 外面统一一个 `keydown`，输入框聚焦时不抢键** | ② 的默写框里打 "1" 也必须进字，不能变成选项 |
| C18 | `TASK.resetV2({ keepCloud: true })` | **`resetV2()` 不收参数**：它本来就只清本地，云端一行都不碰 | 加一个永远为真的"保留云端"选项是假接口，测试直接断言云端行还在即可 |
| C19 | 云测试靠 `addInitScript` 塞假 `window.supabase` | **用 `page.route('**/supabase-js@2*')` 把那个 `<script>` 换成桩本体** | 真脚本是 `defer` 加载的，加载完会**覆盖**先塞进去的假对象 —— 断网时测试"看起来过了"，联网就静默变红。顺带：桩把那条 30s 的 CDN 等待也省掉了（单条 1.8s） |
| C20 | 假 supabase 的行存在内存里 | **存进 `localStorage.__supaRows`** | 「离线攒事件 → 刷新后重放」这条用例必须跨刷新，内存版一刷新就空了，测出来的是假绿 |
| C21 | Task 8 的 `push2` 里 `persist()` / `recompute()` | **`persist2()` / `recompute2()`**，且事件掐到 5000 条时**上传游标 `uploaded` 要一起往前挪** | 游标是数组下标；只掐头不挪游标会把没传的事件当成已传，永久漏传 |

Task 4 的夹具算术同步更正：6 号句只有 1 个词 → 事件数是 `3×2 + 1×1 = 7`，不是 8。

> 离线环境提示：`cdnjs`/`jsdelivr` 不通时每条用例的 `goto` 会等满 ~30s（`load` 事件被 render-blocking 的图标 CSS 拖住），
> 全套 59 条约 13 分钟。这是网络，不是代码 —— 不要为此改产品里的 CDN 引用。
