...
## Summary
验证「无计划 → 设置屏 → 选分钟 → 开始这个计划」这条首屏路径：设置屏只有一颗分钟旋钮（不许把已删的总天数/速度三档加回来）、「今天大约 N 句 · M 题」随选择重算且题数=句数×2、建计划只往事件流里留一条 `dayplan`。
...
只是界面上没入口）；② 换算式被改成"句数 ×1 题"或写死句数；③ 计划多出一个第二真值来源。
...
### 3. 「今天大约」随分钟重算，且题数 = 句数 ×2
按 `.ps-opt` 文本点「60 分钟」，读 `#psSum` 里的句数 `n60` 与题数 `q60`；
再点「5 分钟」，读 `n5` 与 `q5`。
**Verify:** `n5 < n60`；`q60 === n60 * 2`；`q5 === n5 * 2`；当前只有「60 分钟」或「5 分钟」
（最后点的那颗）带 `.sel`。
...
- 不断言队列的具体句数（受句长/实测语速影响），只断言方向与「题数 = 句数 ×2」这类构造关系。
